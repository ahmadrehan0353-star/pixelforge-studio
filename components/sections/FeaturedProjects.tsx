...
export default function FeaturedProjects() {
  const { items } = usePublicCollection<PortfolioProject>("portfolio", fallback, "status");
  ...
  return (
    <section className="section">
      ...
      <div className="mt-16 space-y-10">
        {items.map((project, i) => (
          <Reveal key={project.id ?? project.slug} delay={i * 0.1}>
              ...
              <div className="order-1 flex items-center justify-center lg:order-2">
                {project.liveUrl ? (
                  ...
                  <img
                    ...
                    alt={project.title}
                    className="w-full max-w-md rounded-xl border border-white/[0.1] object-cover shadow-panel"
                  />
                ) : (
