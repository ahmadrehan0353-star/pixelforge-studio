...
export default function Hero() {
  const { data } = usePublicDocument("settings/content", { hero: defaults });
  const content = data.hero;
  const words = content.headline.split(" ");
  const highlightCount = words.length > 3 ? 3 : words.length;
  const leadWords = words.slice(0, words.length - highlightCount).join(" ");
  const highlightWords = words.slice(words.length - highlightCount).join(" ");
    ...
    <section className="relative overflow-hidden pb-24 pt-16 sm:pt-24 lg:pt-28">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-16 px-6 sm:px-8 lg:grid-cols-2 lg:px-10">
        <div>
          ...
          >
            {leadWords && `${leadWords} `}
            <span className="text-gradient">{highlightWords}</span>
          </motion.h1>
